...
export default function MyShelves(props) {
    const { userInfo, handleBookClick } = props;
    const userId = userInfo?.userId;
    const librariesCollectionRef = collection(fireBaseDB, "libraries");
    const [userShelves, setUserShelves] = useState(null);
    const [booksToShow, setBooksToShow] = useState({}); // Initial number of books to show for each shelf
